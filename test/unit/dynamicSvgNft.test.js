...
// these are correct, so where does it go wrong?
    ...
    : describe("Dynamic SVG NFT Unit Tests", () => {
          ...
          describe("mintNft", () => {
              it("emits an event and creates the NFT", async () => {
                  const highValue = ethers.utils.parseEther("1") // $1 per ETH??
                  ...
                  //   const len1 = tokenUri.length
                  //   const len2 = highTokenUri.length
                  //   assert.equal(len1, len2)
                  //   const begin1 = tokenUri.substring(0, 140)
                  //   const begin2 = highTokenUri.substring(0, 140)
                  //   assert.equal(begin1, begin2)
                  //   const end1 = tokenUri.substring(len1 - 150)
                  //   const end2 = highTokenUri.substring(len2 - 150)
                  //   assert.equal(end1, end2)
                  assert.equal(tokenUri, highTokenUri)
              })
              it("shifts the token uri to lower when the price is less than the highvalue", async () => {
                  const highValue = ethers.utils.parseEther("1000000000000") // to the moon price
